...
Action Item: The ReLU activation function is the most common function used for hidden layers. One of the advantages is that it is easy to implement. Another benefit is that it is effective at overcoming the limitations, such as vanishing gradients, of other previously popular activation functions, such as Sigmoid and Tanh. They work best on CNNs and Multilayer perceptrons for hidden layer activation functions. Modesl that uses it are also easier to train and often achieves better performance.
